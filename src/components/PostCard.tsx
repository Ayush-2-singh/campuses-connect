...
export default function PostCard({
  ...
  const handleComment = async () => {
    ...
    const { error } = await supabase
      .from('post_comments')
      .insert({ post_id: post.id, author_id: currentUserId, body: commentText })
    if (error) return
    setCommentText('')
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-end', gap: 6, rowGap: 4, flexShrink: 0 }}
        >
          <span
            style={{
              fontSize: 11,
      ...
      {post.title && (
        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 6px' }}>{post.title}</h3>
      )}
      {editing ? (
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: 'none',
                background: !editBody.trim() || saving ? 'var(--disabled)' : 'var(--accent)',
                color: 'var(--on-accent)',
                fontSize: 13,
                fontWeight: 600,
                cursor: 'pointer',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          <button
            ...
            style={{
              ...
              fontSize: 13,
              fontWeight: 600,
              cursor: !canInteract || joining ? 'default' : 'pointer',
              border: 'none',
              fontFamily: 'inherit',
              background: joined ? 'var(--success)' : 'var(--accent)',
              color: 'var(--on-accent)',
            }}
          >
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 17,
            border: '1px solid var(--border)',
            background: liked ? 'rgba(0,0,0,0.85)' : 'transparent',
        ...
        >
          ...
          {liked ? 'Liked' : 'Like'}
          {likeCount > 0 && (
            <span style={{ fontSize: 12, color: liked ? 'rgba(255,255,255,0.7)' : 'var(--text-muted)' }}>
              {likeCount}
            </span>
          )}
        </button>
        ...
        {isAuthor &&
          !editing &&
          false && ( // edit disabled for admin (they delete, not edit)
            <button
              onClick={() => {
                setEditBody(post.body || '')
                setEditing(true)
        ...
        {/* Delete — bare icon (author or admin) */}
        {canDelete && !editing && (
          <button
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 14 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 10,
                  border: 'none',
                  background: !reportReason.trim() || reportSending ? 'var(--disabled)' : 'var(--danger)',
                  color: 'var(--on-accent)',
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: 'pointer',
        ...
        <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {comments.map((c: any) => (
            <div key={c.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              ...
              >
                <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent)', margin: '0 0 2px' }}>
                  @{c.profiles?.username}
                </p>
              ...
              {(c.author_id === currentUserId || isAdmin) && (
                <button
                  ...
                  style={{
                    ...
                    background: 'none',
                    border: 'none',
                    color: isAdmin && c.author_id !== currentUserId ? 'var(--danger)' : 'var(--text-muted)',
                    fontSize: 13,
                    cursor: 'pointer',
                    padding: '6px',
                    minHeight: 32,
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 20,
                  border: 'none',
                  background: commentText.trim() ? 'var(--accent)' : 'var(--disabled)',
                  color: 'var(--on-accent)',
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: 'pointer',
