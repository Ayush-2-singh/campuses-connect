...
/**
 * FEED VIEW — the full Home/Feed experience (Campus Pulse stats, dashboard,
 * composer, filters, posts). Shared by BOTH routes:
 *
 *   - `/`     → src/app/page.tsx   (default homepage; URL stays clean)
 *   - `/feed` → src/app/feed/page.tsx (legacy/direct links, identical UI)
 *
 * Extracted verbatim from the original feed page so auth, layout, navigation
 * and every behavior stay byte-identical — the route files only mount this.
 */
...
export default function FeedView() {
  ...
  const fetchPostsData = useCallback(
    async (offset: number, currentFilter: string) => {
