...
export default function UserProfilePage() {
    ...
    const load = async () => {
      ...
      if (user && prof) {
        const { data: conn } = await supabase
          ...
          .or(
            `and(requester_id.eq.${user.id},receiver_id.eq.${prof.id}),and(requester_id.eq.${prof.id},receiver_id.eq.${user.id})`
          )
          .limit(1)
        const c = conn?.[0]
        if (c) {
  ...
  const handleConnect = async () => {
    ...
    setConnBusy(false)
    if (error) {
      toast('Could not send the request', { tone: 'danger' })
      return
    }
    ...
    <div data-accent="gold" style={{ minHeight: '100vh', paddingBottom: 80 }}>
      ...
      >
        <div style={{ maxWidth: 640, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <h1 style={{ fontSize: 17, fontWeight: 700, margin: 0, color: 'var(--text-primary)', flex: 1 }}>
            @{profile.username}
          </h1>
          <button
          ...
          >
            <div
              style={{
                position: 'absolute',
                inset: 0,
                background: 'radial-gradient(260px 80px at 85% 20%, rgba(255,255,255,0.35), transparent 70%)',
              }}
            />
          </div>
          <div style={{ padding: '0 24px 24px' }}>
            ...
            >
              ...
              {!isOwnProfile && !user && (
                <button
                  onClick={() =>
                    router.push('/auth/login?redirect=' + encodeURIComponent(`/profile/${profile.username}`))
                  }
                  style={{
                    padding: '8px 18px',
                    borderRadius: 8,
            ...
            {profile.headline && (
              <p style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--accent-text)', margin: '0 0 8px' }}>
                {profile.headline}
              </p>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {posts.length === 0 ? (
              <EmptyState
                icon="message"
                title="No posts yet"
                body="When this person shares something on campus, it will show up here."
              />
            ) : (
              posts.map((post) => {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {notes.length === 0 ? (
              <EmptyState
                ...
                body="Notes this person adds to the library will appear here."
              />
            ) : (
              notes.map((note) => (
                <div
