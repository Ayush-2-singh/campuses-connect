...
export default function AdminCampusesPage() {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      if (!user) {
        router.replace('/auth/login?redirect=/admin/campuses')
        return
      }
  ...
  const handleCreateCollege = async () => {
    ...
    const { error: insErr } = await supabase.from('colleges').insert({
      ...
      email_domains: collegeForm.email_domains
        ? collegeForm.email_domains
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean)
        : null,
  ...
  const handleCreateCampus = async () => {
    if (!selectedCollege || !campusForm.name.trim()) {
      setError('Select a college and enter campus name')
      return
    }
    setSaving(true)
    setError('')
  ...
  const handleCreateDept = async () => {
    if (!selectedCampus || !deptForm.name.trim()) {
      setError('Select a campus and enter department name')
      return
    }
    setSaving(true)
    setError('')
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      alignItems: 'center',
                      justifyContent: 'space-between',
                      borderBottom: colCampuses.length > 0 ? '1px solid var(--border)' : 'none',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '2px 0 0' }}>
                          {col.city || '—'}
                          {col.state ? `, ${col.state}` : ''} · {colCampuses.length} campus
                          {colCampuses.length !== 1 ? 'es' : ''} · {col.slug}
                        </p>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <button
                        ...
                        style={{
                          ...
                          border: 'none',
                          cursor: 'pointer',
                          background: col.is_active ? 'var(--success-light)' : 'var(--danger-light)',
                          color: col.is_active ? 'var(--success-text)' : 'var(--danger)',
                        }}
                      >
                        {col.is_active ? 'Active' : 'Inactive'}
                      </button>
                      <button
                        onClick={() => deleteCollege(col)}
                        style={{
                          padding: '5px 10px',
                          borderRadius: 8,
                          fontSize: 12,
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span
                                  style={{
                                    ...
                                    height: 8,
                                    borderRadius: '50%',
                                    background: camp.is_active ? 'var(--success)' : 'var(--danger)',
                                    flexShrink: 0,
                                  }}
                                />
              ...
              {createTab === 'college' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <input
                      value={collegeForm.email_domains}
                      onChange={(e) => setCollegeForm((f) => ({ ...f, email_domains: e.target.value }))}
                      placeholder="iitd.ac.in, iitd.edu"
                      style={inputStyle}
                    />
                  </div>
                  <button
                    onClick={handleCreateCollege}
                    disabled={saving || !collegeForm.name.trim()}
                    style={{
                      width: '100%',
                      padding: '12px',
                      borderRadius: 10,
                      ...
                      fontFamily: 'inherit',
                      background: saving || !collegeForm.name.trim() ? 'var(--disabled)' : 'var(--accent)',
                      color: 'var(--on-accent)',
                    }}
                  >
              ...
              {createTab === 'campus' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCampusForm((f) => ({ ...f, state: e.target.value }))}
                        placeholder="Uttar Pradesh"
                        style={inputStyle}
                      />
                    </div>
                  ...
                  <button
                    ...
                    disabled={saving || !selectedCollege || !campusForm.name.trim()}
                    style={{
                      width: '100%',
                      padding: '12px',
                      borderRadius: 10,
                      ...
                      fontFamily: 'inherit',
                      background:
                        saving || !selectedCollege || !campusForm.name.trim() ? 'var(--disabled)' : 'var(--accent)',
                      color: 'var(--on-accent)',
                    }}
                  >
              ...
              {createTab === 'department' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {selectedCollege && (
                    <div>
                      ...
                      >
                        ...
                        {campuses
                          .filter((c) => c.college_id === selectedCollege && c.is_active)
                          .map((c) => (
                            <option key={c.id} value={c.id}>
                              {c.name}
                            </option>
                  ...
                  <div>
                    ...
                    <input
                      value={deptForm.short_name}
                      onChange={(e) => setDeptForm((f) => ({ ...f, short_name: e.target.value }))}
                      placeholder="e.g. CSE"
                      style={inputStyle}
                    />
                  </div>
                  <button
                    ...
                    disabled={saving || !selectedCampus || !deptForm.name.trim()}
                    style={{
                      width: '100%',
                      padding: '12px',
                      borderRadius: 10,
                      ...
                      fontFamily: 'inherit',
                      background:
                        saving || !selectedCampus || !deptForm.name.trim() ? 'var(--disabled)' : 'var(--accent)',
                      color: 'var(--on-accent)',
                    }}
                  >
