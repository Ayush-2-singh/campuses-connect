...
export default function TeamsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: open }, { data: mine }, { data: interests }] = await Promise.all([
      ...
      supabase
        .from('team_requests')
        .select('*, team_request_interests(*, profiles(full_name, username, avatar_url))')
  ...
  const openRequestModal = (team: TeamRequest) => {
    if (!user) {
      router.replace(
        '/auth/login?redirect=' + encodeURIComponent(typeof window !== 'undefined' ? window.location.pathname : '')
      )
    ...
    <Layout user={user} profile={profile}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: 0, marginLeft: 34 }}>
              Post for a hackathon, request to join, accept — you&apos;re connected.
            </p>
          </div>
          {user && (
            <button
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <input
                ...
                placeholder="Event/Hackathon name *"
                style={inputStyle}
              />
              <textarea
              ...
              <input
                ...
                placeholder="Skills needed (e.g. React, ML, UI/UX)"
                style={inputStyle}
              />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <input
                  ...
                  placeholder="Contact (WhatsApp/Discord)"
                  style={inputStyle}
                />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              ...
              <button
                onClick={handlePost}
                disabled={!form.event_name || posting}
                style={{
                  flex: 1,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {myRequests.map((t) => (
                ...
                >
                        ...
                        >
                          ...
                          {i.status === 'pending' ? (
                            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                              <button
                                onClick={() => respond(t.id, i.user_id, true)}
                                disabled={busy[`${t.id}:${i.user_id}`]}
                                style={{
                                  padding: '6px 12px',
                                  borderRadius: 8,
                                  border: 'none',
                              ...
                              <button
                                onClick={() => respond(t.id, i.user_id, false)}
                                disabled={busy[`${t.id}:${i.user_id}`]}
                                style={{
                                  padding: '6px 12px',
                                  borderRadius: 8,
                            ...
                            <span
                              style={{
                                ...
                                fontWeight: 600,
                                flexShrink: 0,
                                color: i.status === 'accepted' ? 'var(--success-text)' : 'var(--text-muted)',
                              }}
                            >
                  ...
                  {t.is_open && (
                    <button
                      onClick={() => handleCloseRequest(t.id)}
                      style={{
                        marginTop: 10,
                        fontSize: 12,
                        color: 'var(--text-muted)',
        ...
        {teams.length === 0 ? (
          <EmptyState
            ...
            body={user ? 'Be the first to post!' : 'Join to post or request a spot on a team.'}
          />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {teams.map((team) => {
              ...
              return (
                ...
                >
                  ...
                  {team.contact_info && (
                    <div
                      style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: '8px 12px', fontSize: 13 }}
                    >
                  ...
                  >
                    {isMine ? (
                      <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        Your post · {pendingCount(team)} pending request{pendingCount(team) === 1 ? '' : 's'}
                      </span>
                    ) : status ? (
                      <span
                        style={{
                      ...
                      >
                        {status === 'accepted'
                          ? '🎉 Request accepted — you&apos;re connected!'
                          : status === 'pending'
                            ? '✓ Request sent — waiting for the poster'
                            : 'Request declined'}
                      </span>
                    ) : (
                      <button
      ...
      {requestTarget && (
        <div
          ...
          onClick={() => {
            if (!requestSending) setRequestTarget(null)
          }}
        >
          <div
          ...
          >
            ...
            <p style={{ fontSize: 12.5, color: 'var(--text-muted)', margin: '0 0 14px' }}>
              Join <strong>{requestTarget.event_name}</strong> — the poster will get your request and can accept you.
            </p>
            <textarea
              value={requestMsg}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 14 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 10,
                  border: 'none',
                  background: requestSending ? 'var(--disabled)' : 'var(--accent)',
                  color: 'var(--on-accent)',
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: 'pointer',
