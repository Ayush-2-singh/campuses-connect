  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>We sent a confirmation link to <strong>{email}</strong></p>
        <Link href="/auth/login" style={{ display: 'inline-block', marginTop: 20, color: 'var(--accent)', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>Back to login</Link>
      </div>
    </div>
  )
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="Min. 6 characters" onKeyDown={e => e.key === 'Enter' && handleSignup()} style={inputStyle} />
            </div>
          </div>
