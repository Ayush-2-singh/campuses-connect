...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (selected.college_id)
      supabase
        .from('campuses')
        .select('*')
        .eq('college_id', selected.college_id)
  ...
  useEffect(() => {
    if (selected.campus_id)
      supabase
        .from('departments')
        .select('*')
        .eq('campus_id', selected.campus_id)
  ...
  const btnPrimary = (disabled?: boolean) => ({
    flex: 1,
    background: disabled ? 'var(--disabled)' : 'var(--accent)',
    color: 'var(--on-accent)',
    border: 'none',
    borderRadius: 10,
    padding: '12px',
            ...
            <div style={{ marginTop: 16 }}>
              ...
              >
                ...
                <span style={{ fontSize: 12.5, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                  Open to every student, anywhere in India. Join now — move to your own college the moment it goes live.
                </span>
              </button>
              ...
              <button
                ...
                onClick={next}
                style={{ ...btnPrimary(!selected.college_id), width: '100%', marginTop: 14 }}
              >
                Continue
              </button>
                ...
                <div style={{ marginTop: 10, textAlign: 'center' }}>
                  ...
                  {requestOpen && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 10, textAlign: 'left' }}>
                      ...
                      />
                      <button
                        onClick={requestCollege}
                        disabled={!requestName.trim() || requesting}
                        style={{ ...btnPrimary(!requestName.trim() || requesting), width: '100%' }}
                      >
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 320, overflowY: 'auto' }}>
                {filteredCampuses.map((c) => (
                  <button
                    key={c.id}
                    onClick={() => setSelected((s) => ({ ...s, campus_id: c.id }))}
                ...
                {filteredCampuses.length === 0 && (
                  <p style={{ fontSize: 13, color: 'var(--text-muted)', textAlign: 'center', padding: '16px 0' }}>
                    No campuses match “{q.campus}”. Clear the search to see the full list.
                  </p>
                )}
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                ...
                <button disabled={!selected.campus_id} onClick={next} style={btnPrimary(!selected.campus_id)}>
                  Continue
                </button>
              </div>
            </div>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              >
                {filteredDepartments.map((d) => (
                  <button
                    key={d.id}
                    onClick={() => setSelected((s) => ({ ...s, department_id: d.id }))}
                    style={cardStyle(selected.department_id === d.id)}
                  >
                ...
                {filteredDepartments.length === 0 && (
                  <p style={{ fontSize: 13, color: 'var(--text-muted)', textAlign: 'center', padding: '16px 0' }}>
                    No departments match “{q.department}”. Clear the search to see the full list.
                  </p>
                )}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSelected((s) => ({ ...s, current_year: e.target.value }))}
                ...
                <div>
                  ...
                  <select
                    value={selected.batch_year}
                    onChange={(e) => setSelected((s) => ({ ...s, batch_year: e.target.value }))}
                    style={{ ...inputStyle, padding: '10px 12px' }}
                  >
                    <option value="">Select</option>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  disabled={!selected.department_id || !selected.current_year || !selected.batch_year}
                  onClick={next}
                  style={btnPrimary(!selected.department_id || !selected.current_year || !selected.batch_year)}
                >
                  Continue
                </button>
              </div>
          ...
          {step === 3 && (
            <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSelected((s) => ({ ...s, username: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '') }))
                    }
              ...
              <div>
                ...
                >
                  College Email{' '}
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>(optional — for verified badge)</span>
                </label>
                <input
                  type="email"
                  value={selected.college_email}
                  onChange={(e) => setSelected((s) => ({ ...s, college_email: e.target.value }))}
                  placeholder="you@pwioi.edu.in"
                  style={inputStyle}
                />
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  disabled={!selected.username || loading}
