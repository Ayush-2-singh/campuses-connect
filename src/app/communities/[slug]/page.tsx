...
export default function CommunityPage() {
  ...
  const handleJoin = async () => {
    ...
    if (res === 'pending') {
      ...
      setJoinMsg('Request sent — the owners will review it.')
      return
    }
    if (res === 'test_required') {
      setTestAnswers((community.join_test || []).map(() => -1))
      setTestResult(null)
      setShowTest(true)
      return
    }
  ...
  const submitTest = async () => {
    ...
    if (testAnswers.some((a) => a < 0)) {
      setJoinMsg('Answer every question before submitting.')
      return
    }
    setJoinMsg('')
  ...
  const saveTest = async () => {
    ...
    if (clean.some((t) => t.answer < 0 || t.answer >= t.options.length)) {
      setJoinMsg('Correct-answer index is out of range on a question.')
      return
    }
    setSavingTest(true)
    setJoinMsg('')
    ...
    <Layout user={user} profile={profile}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>
                      ...
                      <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>@{m.profiles?.username}</span>
                    </p>
                  </div>
                  <button
                    onClick={() => reviewMember(m.user_id, true)}
                    style={{
                      padding: '6px 12px',
                      borderRadius: 8,
                      border: 'none',
                  ...
                  <button
                    onClick={() => reviewMember(m.user_id, false)}
                    style={{
                      padding: '6px 12px',
                      borderRadius: 8,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {posts.length === 0 ? (
            <EmptyState
              ...
              body={
                isMember
                  ? 'Owners can kick off the first discussion, resource or announcement in this community.'
                  : 'Join the community to see posts and join in.'
              }
            />
          ) : (
            posts.map((post: any) => (
              <PostCard
                ...
                canInteract={!!user && isMember}
                onChanged={loadPosts}
                isAdmin={isPlatformAdmin || isCommunityAdmin}
              />
            ))
          )}
        </div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {(community.join_test || []).map((q: any, qi: number) => (
                <div key={qi}>
                  <p style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 8px' }}>
                    {qi + 1}. {q.q}
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {(q.options || []).map((opt: string, oi: number) => (
                      <button
                        key={oi}
                        onClick={() => setTestAnswers((a) => a.map((v, i) => (i === qi ? oi : v)))}
                        style={{
                          textAlign: 'left',
                          padding: '9px 12px',
                          borderRadius: 8,
                          border: testAnswers[qi] === oi ? '2px solid var(--accent)' : '1px solid var(--border)',
                          background: testAnswers[qi] === oi ? 'var(--accent-light)' : 'var(--bg)',
                          color: 'var(--text-primary)',
                          fontSize: 13,
                          cursor: 'pointer',
                          fontFamily: 'inherit',
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              ...
              <button
                onClick={submitTest}
                disabled={testSubmitting || testAnswers.some((a) => a < 0)}
                style={{
                  padding: '8px 16px',
                  borderRadius: 10,
                  border: 'none',
                  background: testSubmitting || testAnswers.some((a) => a < 0) ? 'var(--disabled)' : 'var(--accent)',
                  color: 'var(--on-accent)',
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: 'pointer',
      ...
      {showPassword && (
        <div
          ...
          onClick={() => {
            if (!passwordSending) setShowPassword(false)
          }}
        >
          <div
            className="modal-sheet"
            ...
            onClick={(e) => e.stopPropagation()}
            role="dialog"
            aria-label="Enter community password"
          >
            <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 4px' }}>
              🔒 Private community
            </p>
            <p style={{ fontSize: 12.5, color: 'var(--text-muted)', margin: '0 0 14px' }}>
              This community is invite-only. Enter the password to join.
            </p>
            <input
              type="password"
              value={password}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 14 }}>
              ...
              <button
                ...
                disabled={!password || passwordSending}
                style={{
                  padding: '8px 16px',
                  borderRadius: 10,
                  border: 'none',
                  background: !password || passwordSending ? 'var(--disabled)' : 'var(--accent)',
                  color: 'var(--on-accent)',
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: 'pointer',
      ...
      {showTestEditor && (
        <div
          ...
          onClick={() => {
            if (!savingTest) setShowTestEditor(false)
          }}
        >
          <div
            className="modal-sheet"
            ...
            onClick={(e) => e.stopPropagation()}
            role="dialog"
            aria-label="Configure entry test"
          >
            <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 4px' }}>
              📝 Entry test — {community.name}
            </p>
            <p style={{ fontSize: 12.5, color: 'var(--text-muted)', margin: '0 0 14px' }}>
              Students must score 60%+ to join. Leave all questions empty to remove the test.
            </p>
              ...
              {testDraft.map((q, qi) => (
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() => setTestDraft((d) => d.filter((_, i) => i !== qi))}
                      style={{
                        background: 'none',
                        border: 'none',
                        color: 'var(--danger-text)',
                  ...
                  <input
                    type="text"
                    value={q.q}
                    onChange={(e) => setTestDraft((d) => d.map((x, i) => (i === qi ? { ...x, q: e.target.value } : x)))}
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTestDraft((d) =>
                        d.map((x, i) =>
                          i === qi ? { ...x, options: e.target.value.split(',').map((s) => s.trim()), answer: 0 } : x
                        )
                      )
                    }
                    placeholder="Options, comma-separated (at least 2)"
                    style={{ ...inputStyle, marginBottom: 8 }}
                  />
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setTestDraft((d) =>
                          d.map((x, i) => (i === qi ? { ...x, answer: parseInt(e.target.value) } : x))
                        )
                      }
                      style={{
                        flex: 1,
                    ...
                    >
                      {q.options.map((opt, oi) => (
                        <option key={oi} value={oi} disabled={!opt.trim()}>
                          Option {oi + 1}
                          {opt.trim() ? ` — ${opt.trim()}` : ''}
                        </option>
                      ))}
            ...
            <button
              onClick={() => setTestDraft((d) => [...d, emptyTestQ()])}
              style={{
                width: '100%',
                marginTop: 12,
                padding: '9px',
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 10,
                  border: 'none',
                  background: savingTest ? 'var(--disabled)' : 'var(--accent)',
                  color: 'var(--on-accent)',
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: 'pointer',
