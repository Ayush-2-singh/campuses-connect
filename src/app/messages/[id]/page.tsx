...
export default function ChatPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!user) {
        router.replace('/auth/login?redirect=' + encodeURIComponent('/connections?tab=chats'))
        return
      }
      setUser(user)
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '0 0 16px' }}>
            You can only open chats with people you&apos;re connected with.
          </p>
          <button
            onClick={() => router.push('/connections?tab=chats')}
            style={{
              color: 'var(--accent)',
              background: 'none',
              border: 'none',
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {messages.length === 0 && (
          <p style={{ fontSize: 13, color: 'var(--text-muted)', textAlign: 'center', margin: '40px 0' }}>
            ...
            <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start' }}>
              <div
                style={{
                  ...
                  padding: '9px 13px',
                  boxShadow: mine ? 'var(--accent-glow)' : 'var(--shadow-sm)',
                }}
              >
                <p
                  style={{
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 22,
            border: 'none',
            background: !text.trim() || sending ? 'var(--disabled)' : 'var(--accent)',
            color: 'var(--on-accent)',
            fontSize: 14,
            fontWeight: 700,
