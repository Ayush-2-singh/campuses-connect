      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {(['all', 'lost', 'found'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: '7px 18px', borderRadius: 20, fontSize: 13, fontWeight: 500, border: filter === f ? 'none' : '1px solid var(--border)', background: filter === f ? 'var(--accent)' : 'white', color: filter === f ? 'white' : 'var(--text-secondary)', cursor: 'pointer', textTransform: 'capitalize', fontFamily: 'inherit' }}>{f}</button>
          ))}
        </div>
          ...
          <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 14, padding: 20, marginBottom: 20, boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', gap: 10 }}>
                {['lost', 'found'].map(t => (
                  <button key={t} onClick={() => setForm(f => ({ ...f, item_type: t }))} style={{ flex: 1, padding: '10px', borderRadius: 10, border: form.item_type === t ? 'none' : '1px solid var(--border)', background: form.item_type === t ? (t === 'lost' ? '#fef2f2' : '#f0fdf4') : 'white', color: form.item_type === t ? (t === 'lost' ? '#dc2626' : '#15803d') : 'var(--text-secondary)', fontWeight: 600, cursor: 'pointer', textTransform: 'capitalize', fontFamily: 'inherit' }}>{t === 'lost' ? '😢 Lost' : '🎉 Found'}</button>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              ...
              <button onClick={handlePost} disabled={!form.title || posting} style={{ flex: 1, background: posting ? '#93c5fd' : 'var(--accent)', color: 'white', border: 'none', borderRadius: 10, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {filtered.map(item => (
              <div key={item.id} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 14, padding: '18px', boxShadow: 'var(--shadow-sm)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 20, background: item.item_type === 'lost' ? '#fef2f2' : '#f0fdf4', color: item.item_type === 'lost' ? '#dc2626' : '#15803d', fontWeight: 600 }}>{item.item_type === 'lost' ? '😢 Lost' : '🎉 Found'}</span>
